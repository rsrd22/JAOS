 ...
 * @author User
 */
public class ventanaBusquedaConcepto extends javax.swing.JFrame {
     ...
     */
    public ventanaBusquedaConcepto() {
        initComponents();
        modelo = new DefaultTableModel();
        this.setLocationRelativeTo(null);
        ccon = new ControlConcepto();
        vcc = new ventanaConsultarConcepto();
    ...
    public ventanaBusquedaConcepto(ventanaConsultarConcepto vcc) {
...
//        ((DefaultTableCellRenderer)header.getDefaultRenderer()).setHorizontalAlignment(JLabel.CENTER);
    ...
    private void LlenarListaDatos() {
        ...
        SQL = "SELECT * FROM (  \n"
                + "   SELECT  con.`pk_concepto` CODIGO,\n"
                + "   con.`nombre` NOMBRE,\n"
                + "   con.`descripcion` DESCRIPCION,\n"
                + "   con.valor VALOR,\n"
                + "   art.`nombre` ARTICULO,\n"
                + "   IFNULL (con.`fk_tratamiento`,'') TRATAMIENTO\n"
                ...
                + "   con.`descripcion` DESCRIPCION,\n"
                + "   con.valor VALOR,\n"
                + "   con.`fk_articulo` ARTICULO,\n"
                + "   IFNULL (tra.descripcion,'') TRATAMIENTO\n"
                + "   FROM conceptos con INNER JOIN `tratamientos` tra ON con.`fk_tratamiento` =tra.`pk_tratamiento`) \n"
                + "   TABLA ORDER BY NOMBRE ASC";

        SQL = "SELECT con.`pk_concepto` CODIGO,\n"
                + "   con.`nombre` NOMBRE,\n"
                + "   con.`descripcion` DESCRIPCION,\n"
                + "   con.valor VALOR,\n"
                + "   IFNULL(art.`nombre`, 'No asocia Articulo') ARTICULO,\n"
                + "   IFNULL (tra.descripcion, 'No Asocia Tratamiento') TRATAMIENTO\n"
                + "FROM conceptos con\n"
                + "LEFT JOIN tratamientos tra ON tra.`pk_tratamiento` = con.`fk_tratamiento`\n"
                + "LEFT JOIN articulo art ON art.`pk_articulo` = con.`fk_articulo`\n"
                + "ORDER BY NOMBRE ASC;   ";
    ...
    private void initComponents() {
        ...
        fkArticulo = new javax.swing.JLabel();
        ...
        fkArticulo.setText("jLabel7");

        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        setTitle("Busqueda Concepto");
        ...
        jPanel1.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 159, 700, 220));

        lblIni.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        lblIni.setForeground(new java.awt.Color(21, 67, 96));
        lblIni.setText("1");
        jPanel1.add(lblIni, new org.netbeans.lib.awtextra.AbsoluteConstraints(207, 130, 30, -1));
        ...
        lblFin.setText("10");
        jPanel1.add(lblFin, new org.netbeans.lib.awtextra.AbsoluteConstraints(250, 130, 30, -1));
        ...
        jPanel2.add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(660, 0, 30, 30));
        ...
        jPanel2.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(690, 0, 30, 30));

        jLabel4.setFont(new java.awt.Font("Tahoma", 1, 12)); // NOI18N
        jLabel4.setForeground(new java.awt.Color(255, 255, 255));
        jLabel4.setText("Consulta de Conceptos");
        jPanel2.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 10, 152, -1));

        jPanel1.add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 720, 31));
        jPanel1.add(jSeparator1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 100, 250, 20));
        ...
        jPanel1.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 110, 130, 40));
        ...
        try {
            ...
            if (filaSeleccionada > -1) {
                if (vcc != null) {
                    ...
                    //          
                    vcc.lblcodigoConcep.setText("" + modAux.getValueAt(filaSeleccionada, 0));
                    vcc.txtNombreConcepto.setText("" + modAux.getValueAt(filaSeleccionada, 1));
                    vcc.txtaDesconcepto.setText("" + modAux.getValueAt(filaSeleccionada, 2));
                    vcc.txtValor.setText("" + modAux.getValueAt(filaSeleccionada, 3));
                    fkArticulo.setText("" + modAux.getValueAt(filaSeleccionada, 4));

                    vcc.txtNombreConcepto.setVisible(true);

                    vcc.setVisible(true);
                    if (!fkArticulo.getText().isEmpty()) {
                        if(fkArticulo.getText().equalsIgnoreCase("No asocia articulo") && modAux.getValueAt(filaSeleccionada, 5).equals("No Asocia Tratamiento") ){
                                vcc.lblNtratamiento.setText("");
                                vcc.lblNarticulo.setText("");
                                vcc.lblTipoTratamiento.setVisible(false);
                                vcc.cbTipoTratamientos.setVisible(false);
                            }
                            else if (fkArticulo.getText().equalsIgnoreCase("No asocia articulo")) {
                                vcc.rbTratamientos.setSelected(true);
                                vcc.lblNtratamiento.setText("" + modAux.getValueAt(filaSeleccionada, 5));
                                vcc.lblNarticulo.setText("");
                                vcc.lblTipoTratamiento.setVisible(false);
                                vcc.cbTipoTratamientos.setVisible(false);
                            } else {
                                vcc.rbArticulos.setSelected(true);
                                vcc.lblNarticulo.setText("" + modAux.getValueAt(filaSeleccionada, 4));
                                vcc.lblNomArticulos.setVisible(false);
                                vcc.cbArticulos.setVisible(false);
                                vcc.lblNtratamiento.setText("");
                            }
                    }
                    vcc.ConceptosEstadoFormulario(3);
...
//        this.dispose();
    ...
    private void jLabel6MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel6MouseClicked
        this.setState(ventanaBusquedaConcepto.ICONIFIED);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ventanaBusquedaConcepto.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ventanaBusquedaConcepto(null).setVisible(true);
            }
        });
    }
    ...
    public javax.swing.JLabel fkArticulo;
    ...
    private void GenerarTablaOLD() {
        try {
            ArrayList<String[]> datos = new ArrayList<>();
            String filtro = Utilidades.CodificarElemento(txtFiltro.getText());
            String add = "WHERE CONCAT_WS(' ',LOWER(CODIGO), LOWER(NOMBRE), LOWER(DESCRIPCION),LOWER(VALOR),LOWER(ARTICULO),LOWER(TRATAMIENTO)) LIKE '%" + filtro + "%' ";
//            switch (opc) {//PERSONAL
//                case 1: {
            SQL = "SELECT * FROM (  \n"
                    + "                   SELECT  con.`pk_concepto` CODIGO,\n"
